...
2. 同期対象のファイル/ディレクトリを GitHub API を使って Template から取得する
   ...
   - 各ファイルの SHA ハッシュ情報も取得
3. ローカルファイルと Template ファイルの SHA を比較する
   - SHA が一致する場合：変更なしとしてスキップ（ダウンロード不要）
   - SHA が異なる場合または新規ファイルの場合：ダウンロードして反映
4. 継承先リポジトリの同名パスへ反映する（同期対象"だけ"を更新する）
5. `delete_if_missing: true` の対象について、Template に存在しないが継承先に存在するファイルは削除する
6. 変更がある場合のみ、以下の処理を行う：
...
- GitHub API から取得したファイルの SHA ハッシュと、ローカルファイルの SHA を比較
- SHA が一致する場合はダウンロードをスキップ（ネットワーク通信を最小化）
- ローカルファイルの SHA は `git hash-object` コマンドで計算
...
- 同期対象のファイル取得中にエラーが発生した場合：**ロールバックして停止**（中途半端な状態で PR を作成しない）
